/*!
 * @file Adafruit_BLE_UART.cpp
 *
 * @mainpage Adafruit nRF8001 Library
 *
...
#else
#define NUMBER_OF_PIPES 0 //!< How many pipes
...
/*! This is the Uart RX buffer, which we manage internally when data is
 * available! */
#define ADAFRUIT_BLE_UART_RXBUFFER_SIZE 64
uint8_t
    adafruit_ble_rx_buffer[ADAFRUIT_BLE_UART_RXBUFFER_SIZE]; //!< BLE uart RX
                                                             //!< buffer size
volatile uint16_t adafruit_ble_rx_head; //!< BLE RX buffer head
volatile uint16_t adafruit_ble_rx_tail; //!< BLE RX buffer tail

int8_t HAL_IO_RADIO_RESET, //!< Radio reset
    HAL_IO_RADIO_REQN,     //!< Radio Reqn
    HAL_IO_RADIO_RDY,      //!< Radio ready
    HAL_IO_RADIO_IRQ;      //!< Radio irq
...
void Adafruit_BLE_UART::defaultRX(uint8_t *buffer, uint8_t len) {
  for (int i = 0; i < len; i++) {
    uint16_t new_head =
        (uint16_t)(adafruit_ble_rx_head + 1) % ADAFRUIT_BLE_UART_RXBUFFER_SIZE;
...
/*!
 * @brief Constructor for the UART service
 * @param req Req pin
 * @param rdy Ready pin
 * @param rst Reset pin
 */
...
/*!
 * @brief Transmits data out via the TX characteristic (when available)
...
#ifdef BLE_RW_DEBUG
  Serial.print(F("\tWriting one byte 0x"));
  ...
  if (lib_aci_event_get(&aci_state, &aci_data)) {
    ...
    case ACI_EVT_CMD_RSP:
      ...
      if (ACI_STATUS_SUCCESS != aci_evt->params.cmd_rsp.cmd_status) {
        // ACI ReadDynamicData and ACI WriteDynamicData will have status codes
        // of TRANSACTION_CONTINUE and TRANSACTION_COMPLETE all other ACI
        // commands will have status code of ACI_STATUS_SUCCESS for a successful
        // command
        if (debugMode) {
      ...
      if (ACI_CMD_GET_DEVICE_VERSION == aci_evt->params.cmd_rsp.cmd_opcode) {
        ...
        lib_aci_set_local_data(
            &aci_state, PIPE_DEVICE_INFORMATION_HARDWARE_REVISION_STRING_SET,
    ...
    case ACI_EVT_CONNECTED:
      ...
      /* Get the device version of the nRF8001 and store it in the Hardware
       * Revision String */
    ...
    case ACI_EVT_PIPE_STATUS:
      if (lib_aci_is_pipe_available(&aci_state,
        ...
        lib_aci_change_timing_GAP_PPCP(); // change the timing on the link as
                                          // specified in the nRFgo studio ->
                                          // nRF8001 conf. -> GAP. Used to
    ...
    case ACI_EVT_DATA_RECEIVED:
      defaultRX(aci_evt->params.data_received.rx_data.aci_data,
                aci_evt->len - 2);
      if (rx_event)
    ...
    case ACI_EVT_PIPE_ERROR:
      /* See the appendix in the nRF8001 Product Specication for details on the
       * error codes */
      if (debugMode) {
    ...
    // command queue the arduino can go to sleep Arduino can go to sleep now
 ...
 * @param advTimeout The advertising timeout in seconds (0 = infinite
 * advertising)
 * @param advInterval The delay between advertising packets in 0.625ms units
