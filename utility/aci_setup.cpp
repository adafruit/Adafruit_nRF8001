...
aci_status_code_t aci_setup(aci_state_t *aci_stat, uint8_t num_cmds,
                            uint8_t num_cmd_offset) {
  uint8_t i = 0;
  uint8_t evt_count = 0;
  ...
  while (i < num_cmds) {
    ...
    // Add 2 bytes to the length byte for status byte, length for the total
    // number of bytes
    memcpy_P(&aci_cmd,
             &(aci_stat->aci_setup_info.setup_msgs[num_cmd_offset + i]),
             pgm_read_byte_near(
                 &(aci_stat->aci_setup_info.setup_msgs[num_cmd_offset + i]
                       .buffer[0])) +
                 2);
...
// Serial.println(i, DEC);
#ifdef __arm__
      // This entire setup scheme may have an off-by-one error, where it tries
      // to put 8 commends into the queue which can only hold 7 due to the way
      // the head & tail indexes are managed.  On AVR, the processor is simply
      // too slow to fill the queue before at least one interrupt, but a fast
      // ARM processor can easily do so.  This delay is a workaround to avoid
      // having to restructure a lot of code...
  ...
  while (1) {
    // We will sit here if we do not get the same number of command response
    // evts as the commands sent to the ACI
    //
    //@check The setup wil fail in the while(1) below when the 32KHz source for
    //the nRF8001 is in-correct in the setup generated in the nRFgo studio
...
aci_status_code_t do_aci_setup(aci_state_t *aci_stat) {
  ...
  if (ACI_QUEUE_SIZE >= aci_stat->aci_setup_info.num_setup_msgs) {
    ...
    for (i = 0; i < (aci_stat->aci_setup_info.num_setup_msgs / ACI_QUEUE_SIZE);
         i++) {
    ...
    if ((aci_stat->aci_setup_info.num_setup_msgs % ACI_QUEUE_SIZE) != 0) {
      status = aci_setup(
          aci_stat, aci_stat->aci_setup_info.num_setup_msgs % ACI_QUEUE_SIZE,
          (ACI_QUEUE_SIZE * i));
    }
  }
