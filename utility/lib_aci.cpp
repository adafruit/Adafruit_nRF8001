...
static uint16_t cx_rf_interval; // rf_interval = cx_rf_interval * 1.25 ms
...
// The following structure (aci_cmd_params_open_adv_pipe) will be used to store
// the complete command including the pipes to be opened.
...
uint16_t lib_aci_get_cx_interval_ms(aci_state_t *aci_stat) {
  ...
  cx_rf_interval_ms_32bits *=
      125; // the connection interval is given in multiple of 0.125 milliseconds
...
bool lib_aci_set_app_latency(uint16_t latency,
  ...
  acil_encode_cmd_set_app_latency(&(msg_to_send.buffer[0]),
                                  &aci_set_app_latency);
...
bool lib_aci_set_local_data(aci_state_t *aci_stat, uint8_t pipe,
                            uint8_t *p_value, uint8_t size) {
  ...
  acil_encode_cmd_set_local_data(&(msg_to_send.buffer[0]),
                                 &aci_cmd_params_set_local_data, size);
  return hal_aci_tl_send(&msg_to_send);
}
...
bool lib_aci_disconnect(aci_state_t *aci_stat, aci_disconnect_reason_t reason) {
  ...
  acil_encode_cmd_disconnect(&(msg_to_send.buffer[0]),
                             &aci_cmd_params_disconnect);
...
bool lib_aci_set_tx_power(aci_device_output_power_t tx_power) {
  ...
  acil_encode_cmd_set_radio_tx_power(&(msg_to_send.buffer[0]),
...
bool lib_aci_send_data(uint8_t pipe, uint8_t *p_value, uint8_t size) {
  ...
  }
  {
    ...
    acil_encode_cmd_send_data(&(msg_to_send.buffer[0]),
                              &aci_cmd_params_send_data, size);
...
bool lib_aci_request_data(aci_state_t *aci_stat, uint8_t pipe) {
  ...
  if (!((p_services_pipe_type_map[pipe - 1].location == ACI_STORE_REMOTE) &&
        (p_services_pipe_type_map[pipe - 1].pipe_type == ACI_RX_REQ))) {
    return false;
  }

  {

    {
      ...
      acil_encode_cmd_request_data(&(msg_to_send.buffer[0]),
                                   &aci_cmd_params_request_data);
...
bool lib_aci_change_timing(uint16_t minimun_cx_interval,
  ...
  aci_cmd_params_change_timing.conn_params.min_conn_interval =
      minimun_cx_interval;
  aci_cmd_params_change_timing.conn_params.max_conn_interval =
      maximum_cx_interval;
  ...
  acil_encode_cmd_change_timing_req(&(msg_to_send.buffer[0]),
                                    &aci_cmd_params_change_timing);
  return hal_aci_tl_send(&msg_to_send);
}
...
bool lib_aci_open_remote_pipe(aci_state_t *aci_stat, uint8_t pipe) {
...

  {
    ...
    acil_encode_cmd_open_remote_pipe(&(msg_to_send.buffer[0]),
                                     &aci_cmd_params_open_remote_pipe);
    ret_val = hal_aci_tl_send(&msg_to_send);
  }
  return ret_val;
}
...
bool lib_aci_close_remote_pipe(aci_state_t *aci_stat, uint8_t pipe) {
...

  {
    ...
    acil_encode_cmd_close_remote_pipe(&(msg_to_send.buffer[0]),
                                      &aci_cmd_params_close_remote_pipe);
    ret_val = hal_aci_tl_send(&msg_to_send);
  }
  return ret_val;
}
...
bool lib_aci_echo_msg(uint8_t msg_size, uint8_t *p_msg_data) {
  ...
  acil_encode_cmd_echo_msg(&(msg_to_send.buffer[0]), &aci_cmd_params_echo,
                           msg_size);
  ...
  */
  {
    ...
    case ACI_EVT_PIPE_STATUS: {
      ...
      for (i = 0; i < PIPES_ARRAY_SIZE; i++) {
        aci_stat->pipes_open_bitmap[i] =
            aci_evt->params.pipe_status.pipes_open_bitmap[i];
        aci_stat->pipes_closed_bitmap[i] =
            aci_evt->params.pipe_status.pipes_closed_bitmap[i];
      }
    } break;
...
bool lib_aci_broadcast(const uint16_t timeout, const uint16_t adv_interval) {
  ...
  // The adv_interval should be between 160 and 16384 (which translates to the
  // advertisement interval values 100 ms and 10.24 s.
  ...
  acil_encode_cmd_broadcast(&(msg_to_send.buffer[0]),
                            &aci_cmd_params_broadcast);
  return hal_aci_tl_send(&msg_to_send);
}
...
bool lib_aci_open_adv_pipes(const uint8_t *const adv_service_data_pipes) {
  ...
  acil_encode_cmd_open_adv_pipes(&(msg_to_send.buffer[0]),
...
bool lib_aci_write_dynamic_data(uint8_t sequence_number, uint8_t *dynamic_data,
                                uint8_t length) {
  acil_encode_cmd_write_dynamic_data(&(msg_to_send.buffer[0]), sequence_number,
                                     dynamic_data, length);
  return hal_aci_tl_send(&msg_to_send);
}
...
void lib_aci_flush(void) { m_aci_q_flush(); }

void lib_aci_debug_print(bool enable) { hal_aci_debug_print(enable); }
