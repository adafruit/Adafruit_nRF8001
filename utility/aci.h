 ...
 *
 * Data flow control:
 * The flow control is credit based and the credit is initally given using the
 * "device started" event. A credit of more than 1 is given to the application
 * mcu. These credits are used only after the "ACI Connected Event" is sent to
 * the application mcu.
 *
 * every send_data that is used decrements the credit available by 1. This is to
 * be tracked by the application mcu. When the credit available reaches 0, the
 * application mcu shall not send any more send_data. Credit is returned using
 * the "credit event", this returned credit can then be used to send more
 * send_data. This flow control is not necessary and not available for
 * Broadcast. The entire credit available with the external mcu expires when a
 * "disconnected" event arrives.
 *
 * Command flow control:
 * When a command is sent over the ACI, the next command shall not be sent until
 * after a response for the command sent has arrived.
 *
 */
...
/**
 * @def ACI_VERSION
 * @brief Current ACI protocol version. 0 means a device that is not yet
 * released. A numer greater than 0 refers to a specific ACI version documented
 * and released. The ACI consists of the ACI commands, ACI events and error
 * codes.
 */
 ...
 * @brief Maximum length in bytes of a full ACI packet, including length prefix,
 * opcode and payload
 */
...
typedef enum {
  ...
  ACI_DEVICE_OUTPUT_POWER_0DBM =
      0x03 /**< Output power set to 0dBm   - DEFAULT*/
...
typedef enum {
  ACI_REASON_TERMINATE =
      0x01, /**< Use this to disconnect (does a terminate request), you need to
               wait for the "disconnected" event */
  ACI_REASON_BAD_TIMING =
      0x02 /*<Use this to disconnect and inform the peer, that the timing on the
              link is not acceptable for the device, you need to wait for the
              "disconnected" event */
 ...
 */
#define ACI_VS_UUID_128_MAX_COUNT                                              \
  64 /** #0 reserved for invalid, #1 reservered for BT SIG and a maximum of    \
        1024 bytes (16*64) */
...
typedef struct {
  uint16_t
      min_conn_interval; /**< Minimum connection interval requested from peer */
...
#define ACI_PPCP_MIN_CONN_INTVL_MAX 0x0C80
  uint16_t
      max_conn_interval; /**< Maximum connection interval requested from peer */
...
#define ACI_PPCP_MAX_CONN_INTVL_MAX 0x0C80
  uint16_t
      slave_latency; /**< Connection interval latency requested from peer */
#define ACI_PPCP_SLAVE_LATENCY_MAX 0x03E8
  uint16_t timeout_mult; /**< Link supervisor timeout multiplier requested from
                            peer */
...
typedef enum {
  ...
  /**
   * Bonding error: Timeout can occur when link termination is unexpected or did
   * not get connected OR SMP timer expired
   */
...
typedef enum {
  ...
  /**
   * Pipe Error Event : Data size exceeds size specified for pipe : Transmit
   * failed
   */
