 ...
 *  @details A hook function that must be implemented by the client of this
 * module. The function will be called by this module when a new message has
 * been received from the nRF8001.
 *  @param received_msg Pointer to a structure containing a pointer to the
 * received data.
 */
 ...
 *  @details
 *  This function sends an ACI command to the radio. This will memorize the
 * pointer of the message to send and lower the request line. When the device
 * lowers the ready line, @ref hal_aci_tl_poll_rdy_line() will send the data.
 *  @param aci_buffer Pointer to the message to send.
 *  @return True if the send is started successfully, false if a transaction is
 * already running.
 */
 ...
 *  @details
 *  Call this function from the main context at regular intervals to check if
 * the nRF8001 RDYN line indicates a pending transaction. If a transaction is
 * pending, this function will treat it and call the receive hook.
 */
 ...
 * Serial. When the enable parameter is false. The debug printing is disabled on
 * the Serial. By default the debug printing is disabled.
 */
