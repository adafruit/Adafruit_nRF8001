 ...
 */
typedef struct {
  uint8_t device_mode; /**< enum aci_device_operation_mode_t -> Mode in which
                          the device is being started */
  uint8_t hw_error; /**< enum aci_hw_error_t -> Hardware Error if available for
                       the start */
  uint8_t credit_available; /**< Flow control credit available for this specific
                               FW build */
 ...
 * @brief Structure for the ACI_EVT_CMD_RSP event with opcode=ACI_CMD_DTM_CMD
 ...
 * @brief Structure for the ACI_EVT_CMD_RSP event with
 * opcode=ACI_CMD_READ_DYNAMIC_DATA event return parameters
 * @note Dynamic data chunk size in this event is defined to go up to
 * ACI_PACKET_MAX_LEN - 5
 */
typedef struct {
  uint8_t seq_no;
 ...
 * @brief Structure for the ACI_EVT_CMD_RSP event with
 * opcode=ACI_CMD_GET_DEVICE_VERSION event return parameters
 */
typedef struct {
 ...
 * @brief Structure for the ACI_EVT_CMD_RSP event with
 * opcode=ACI_CMD_GET_DEVICE_ADDRESS event return parameters
 */
typedef struct {
 ...
 * @brief Structure for the ACI_EVT_CMD_RSP event with
 * opcode=ACI_CMD_GET_BATTERY_LEVEL event return parameters
 */
typedef struct {
 ...
 * @brief Structure for the ACI_EVT_CMD_RSP event with
 * opcode=ACI_CMD_GET_TEMPERATURE event return parameters
 */
typedef struct {
 ...
 */
typedef struct {
  uint8_t cmd_opcode; /**< enum aci_cmd_opcode_t -> Command opcode for which the
                         event response is being sent */
  uint8_t cmd_status; /**< enum aci_status_code_t -> Status of the command that
                         was sent. Used in the context of the command. */
  union {
...
typedef struct {
  ...
  uint16_t conn_rf_interval;      /**< rf_interval = conn_rf_interval * 1.25 ms
                                     Range:0x0006 to 0x0C80 */
  uint16_t conn_slave_rf_latency; /**< Number of RF events the slave can skip */
  uint16_t conn_rf_timeout; /**< Timeout as a multiple of 10ms i.e timeout =
                               conn_rf_timeout * 10ms Range: 0x000A to 0x0C80 */
  uint8_t master_clock_accuracy; /**< enum aci_clock_accuracy_t -> Clock
                                    accuracy of Bluetooth master: Enumerated
                                    list of values from 500 ppm to 20 ppm */
