...
/** @addtogroup lib_aci
@{
@brief Library for the logical part of the Application Controller Interface
(ACI)
*/
...
typedef struct aci_state_t {
  aci_setup_info_t
      aci_setup_info; /* Data structures that are created from nRFgo Studio */
  uint8_t bonded; /* ( aci_bond_status_code_t ) Is the nRF8001 bonded to a peer
                     device */
  uint8_t data_credit_total; /* Total data credit available for the specific
                                version of the nRF8001, total equals available
  ...
  uint8_t data_credit_available; /* Available data credits at a specific point
                                    of time, ACI_EVT_DATA_CREDIT updates the
                                    available credits */

  uint16_t connection_interval; /* Multiply by 1.25 to get the connection
                                   interval in milliseconds*/
  uint16_t slave_latency; /* Number of consecutive connection intervals that the
                             nRF8001 is not required to transmit. Use this to
                             save power */
  uint16_t supervision_timeout; /* Multiply by 10 to get the supervision timeout
                                   in milliseconds */

  uint8_t pipes_open_bitmap[PIPES_ARRAY_SIZE];   /* Bitmap -> pipes are open and
                                                    can be used for sending data
                                                    over the air */
  uint8_t pipes_closed_bitmap[PIPES_ARRAY_SIZE]; /* Bitmap -> pipes are closed
                                                    and cannot be used for
                                                    sending data over the air */
  bool confirmation_pending; /* Attribute protocol Handle Value confirmation is
                             pending for a Handle Value Indication (ACK is
                             pending for a TX_ACK pipe) on local GATT Server*/
...
/** @brief Function to enable printing of all ACI commands sent and ACI events
 received
 *  @details This function shall be used to enable or disable the debug
 printing. Debug printing is disabled by default.
 */
 ...
 * function will reset all the variables locally used by ACI library to their
 ...
 *  If the radio is advertising or connected, it sends back an error, then use
 * lib_aci_radio_reset if advertising or disconnect if in a connection.
 ...
 *  @details
 *  This command updates the value of the characteristic value or the
 * characteristic descriptor stored locally on the device. Can be called for all
 * types of pipes as long as the data is stored locally.
 ...
 *  @return True if the transaction is successfully initiated.
 */
bool lib_aci_set_local_data(aci_state_t *aci_stat, uint8_t pipe, uint8_t *value,
                            uint8_t size);

/** @brief Sends Broadcast message to the radio.
 *  @details The Broadcast message starts advertisement procedure
 *  using the given interval with the intention of broadcasting data to a peer
 * device.
 *  @param timeout Time, in seconds, to advertise before exiting to standby mode
 * (0 means infinite). Valid values: 0 to 16383.
 *  @param adv_interval Advertising interval (in multiple of 0.625&nbsp;ms).
 *  Valid values: 160 to 16384 (which corresponds to an interval from 100 ms
 * to 10.24 s).
...
/** @brief Sends a command to the radio to set the input pipe to be placed in
 * Advertisement Service Data.
 *  @details This function sends a command to the radio that places the pipe in
 *  advertisement service data.  To start advertising service data, call this
 * function before Connecting, Broadcasting or Bonding to peer. The data that
 * should be sent in the advertisement packets must be set using the @c
 * lib_aci_set_local_data function. This function can be called during
...
/** @brief Sends a command to the radio to set the pipes to be placed in
 * Advertisement Service Data.
 *  @details This function will send a command to the radio that will set the
 * pipes to be placed in advertisement Service Data.  To start advertising
 * service data, this function should be called before Connecting, Broadcasting
 * or Bonding to peer. This function can be called during advertising to
 * enable/disable broadcast pipes. Use this as an alternative to @ref
 * lib_aci_open_adv_pipe to avoid multiple function calls for placing multiple
 * pipes in the adv data.
 *  @param adv_service_data_pipes Pipe bitmap, where '1' indicates that the
 * corresponding Valid Values: 0000000000000000 to FEFFFFFFFFFFFF7F (See the ACI
 * Pipe Status Evt bitmap in the nRF8001 datasheet TX_BROADCAST pipe data is to
 * be placed in Advertising Service Data fields
 *  @return true if the Open Adv Pipe message was sent successfully to the
 * radio.
 */
 ...
 *  @details This function sends a @c ChangeTiming command to the radio.  This
 * command triggers a "L2CAP change connection parameters" request to the
 * master. If the master rejects or accepts but doesn't change the connection
 * parameters within 30 seconds, a timing event with the unchanged connection
 * parameters is sent by the radio. If the request is accepted and the master
 * changes connection parameters, a timing event with the new connection
 * parameters is sent by the radio. If the master doesn't reply to the request
 * within 60 seconds, the radio disconnects.
 *  @param minimun_cx_interval Minimum connection interval requested, in
 * multiple of 1.25&nbsp;ms.
 *  @param maximum_cx_interval Maximum connection interval requested, in
 * multiple of 1.25&nbsp;ms.
...
bool lib_aci_change_timing(uint16_t minimun_cx_interval,
                           uint16_t maximum_cx_interval, uint16_t slave_latency,
                           uint16_t timeout);

/** @brief Sends a L2CAP change connection parameters request with the
 * connection predefined preffered connection parameters.
 *  @details This function sends a @c ChangeTiming command to the radio. This
 * command triggers a "L2CAP change connection parameters" request to the
 * master. If the master rejects or accepts but doesn't change the connection
 * parameters within 30 seconds, a timing event with the unchanged connection
 ...
 * changes connection parameters, a timing event with the new connection
 * parameters is sent by the radio. If the master doesn't reply to the request
 * within 60 seconds, the radio disconnects. The timing parameters used are the
 * Timing parameters in the GAP settings in the nRFgo Studio. The Timing
 * parameters as stored as the GAP Preferred Peripheral Connection Parameters.
 *  @return True if the transaction is successfully initiated.
 */
bool lib_aci_change_timing_GAP_PPCP(void);

/** @brief Sends acknowledgement message to peer.
 *  @details This function sends @c SendDataAck command to radio. The radio is
 * expected to send either Handle Value Confirmation or Write response depending
 ...
 *  @details This function sends @c SendDataNack command to radio. The radio is
 * expected to send Error Response to the peer.
 ...
 */
bool lib_aci_send_nack(aci_state_t *aci_stat, const uint8_t pipe,
                       const uint8_t error_code);

/** @brief Sends ReadDynamicData command to the host.
 *  @details This function sends @c ReadDynamicData command to host. The host is
 * expected to send @c CommandResponse back with the dynamic data. The
 * application is expected to call this function in a loop until all the dynamic
 * data is read out from the host. As long as there is dynamic data to be read
 * from the host, the command response for this message has its status field set
 * to ACI_STATUS_TRANSACTION_CONTINUE (0x01). The application may chose to store
 * this read out data in a non-volatile memory location and later chose to write
 * it back using the function lib_aci_write_dynamic_data.
 ...
 * is expected to send @c CommandResponse with the status of this operation. As
 * long as the status field in the @c CommandResponse is
 * ACI_STATUS_TRANSACTION_CONTINUE (0x01), the hosts expects more dynamic data
 * to be written. This function should ideally be called in a cycle, until all
 * the stored dynamic data is sent to the host. This function should be called
 * with the dynamic data obtained from the response to a @c ReadDynamicData (see
 * @c lib_aci_read_dynamic_data) command.
 ...
 *  master rejects with a pairing failed or if the bond timer expires the
 ...
 *  The queue is updated by the SPI driver for the ACI running in the interrupt
 * context
